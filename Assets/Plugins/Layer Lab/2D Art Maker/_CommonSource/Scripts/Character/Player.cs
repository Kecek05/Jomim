...
namespace LayerLab.ArtMaker
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        private void HandleMouseInput()
        {
            ...
            if ((UnityEngine.Input.GetMouseButtonDown(0) || UnityEngine.Input.GetMouseButtonDown(1)) && !isOverUI)
            {
                _isDragging = true;
            }

            if ((UnityEngine.Input.GetMouseButton(0) || UnityEngine.Input.GetMouseButton(1)) && _isDragging)
            {
                var mousePosition = CameraControl.Instance.Cam.ScreenToWorldPoint(UnityEngine.Input.mousePosition);
